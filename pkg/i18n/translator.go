...
// submodule §11.4.36). It externalises every user-facing CLI string the
// docprocessor binary prints so a Serbian, Japanese, or Spanish operator
// receives a localised rendering instead of a verbatim English literal.
//
...
// DocProcessor uses it for every CONST-046-migrated user-facing string.
...
// the CLI remains functional in stripped-down environments (CI smoke
// builds, integration harnesses that exercise wire format only) and
// absence-of-bundle is loudly visible in captured output.
//
